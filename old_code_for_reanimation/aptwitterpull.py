...
class Aptwitterpull:
  ...
  # Original code from https://labsblog.f-secure.com/2018/02/16/searching-twitter-with-twarc/
  ...
  def pull_data_for_target(self, count, target):
    ...
    if len(target) < 30:
      save_dir = '{}/{}'.format(self.data_dir, target.replace(" ", "_"))
    else:
      save_dir = '{}/target_{}'.format(self.data_dir, count + 1)
  ...
  def pull_data_for_targets(self, new_target_list=[], new_data_dir=''):
    # Meat of the class
    ...
    for count, target in enumerate(target_list):
      self.pull_data_for_target(count, target)
...
if __name__ == '__main__':
  ...
  tp.pull_data_for_targets(target_list)
